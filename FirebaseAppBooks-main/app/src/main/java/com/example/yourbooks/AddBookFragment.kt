...
class AddBookFragment : Fragment() {
        ...
        context?.let { context ->
            ...
            val adapter: ArrayAdapter<String> = object : ArrayAdapter<String>(
                ...
                override fun getDropDownView(
                    ...
                    view.setTextColor(Color.parseColor("#087f23"))

                    if (position == spinnerIsRead.selectedItemPosition) {
                        view.background = ColorDrawable(Color.parseColor("#994caf50"))
                    }
                    return view
                }
    ...
    private fun setupAddButton(v:View) {
        buttonAdd.setOnClickListener {
            ...
            {
                Snackbar.make(requireView(), "Podaj autora", Snackbar.LENGTH_SHORT).show()
            }
            else if(editTextTitle.text.toString()=="")
            {
                Snackbar.make(requireView(), "Podaj tytuł", Snackbar.LENGTH_SHORT).show()
            }
            else
            {
                ...
                currentUser.child(id).setValue(book).addOnCompleteListener{
                    ...
                    Snackbar.make(requireView(), "Książka dodana.", Snackbar.LENGTH_SHORT).show()
                }
            }
        }
    }
